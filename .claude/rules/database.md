...
pinned at `tests/scripts/test_tenancy_gate.py:377`):
...
- a nullable parameter tested alone: write `CAST(:p AS text) IS NULL`
  ...
  (`provisioning.connect_destination`'s lookup, `:723`);
...
- Schema DDL is **advertised**: the target lineage must stay an ordered prefix
  ...
  `scripts/advertised_ddl_manifest.json` (`advertised_ddl.target_lineage_files`).
