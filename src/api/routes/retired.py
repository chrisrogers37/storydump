...
it, and the deployment fact that seemed to make that free — the legacy bot had
not run since the target root was armed (2026-08-24), so it could not SEND a
new Mini App button (and it is gone altogether since #1216) — does not cover
the buttons it already sent. Its `build_webapp_button` (the deleted `src/services/core/telegram_utils.py`)
