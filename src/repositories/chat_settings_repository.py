...
class ChatSettingsRepository(BaseRepository):
    ...
    def get_by_chat_id(self, telegram_chat_id: int) -> Optional[ChatSettings]:
        ...
        None-in is None-out, and the guard is tenant isolation rather than
        pedantry: SQLAlchemy compiles ``== None`` to IS NULL, so with
        ``telegram_chat_id`` nullable a None here would return an ARBITRARY
        personal (chat-less) tenant — another tenant's settings served for a
        missing chat id. Fail-closed, matching the doctrine ``get_by_id``
        states (#841). ``require_by_chat_id`` turns the None into its typed
        ``unknown_binding`` refusal.
        """
