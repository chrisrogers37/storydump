...
to its new chat id when Telegram announces the upgrade; if that notice never
arrived, the next delivery finds the move and follows it instead (that one
message fails). Only a 401 from Telegram means the credential itself died.
