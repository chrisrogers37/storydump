...
`_register_webhook`, idempotent on every deploy; `/health` reports the result under `webhook`,
`TARGET_TELEGRAM_WEBHOOK_AUTOREGISTER=0` switches it off). This tool remains for `verify`,
