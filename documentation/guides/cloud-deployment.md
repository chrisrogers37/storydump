...
| A deploy fails its health check with `FATAL: the credential key ring cannot load` (worker) or `Refusing to start` (API) | `ENCRYPTION_KEY` is unset on that service or not a Fernet key; the message says which. Set the key the other service holds — a NEW key cannot read the stored credentials. For a first install, generate one: `python -c "from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())"` |
