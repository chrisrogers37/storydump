...
> **Archived 2026-09-20 — SUPERSEDED (executed differently than written).** The rehearsal protocol was never run as specified: steps 3a–3d were applied by hand on 2026-08-24/26 (`00` FC-7 §7), 3e was abandoned, and 3f/3g/step 8 ran as migrations 078 (2026-09-18) and 079/080 (2026-09-19, the legacy tear-out #1216, phase 04), each rehearsed first on a fresh Neon PITR branch under `documentation/operations/legacy-window-close.md`; 080 is the success-variant stand-down, partial by fork F8 (a). The backout was a PITR marker branch, not the D41 lever; the DR-drill legs were not recorded; no run report was ever filed under `runs/` — the record is `../../planning/2026-09-16-legacy-tear-out/RUN_LOG.md`. Neon's PITR window measured 24 h on 2026-09-18, not the ≥ 7 d `05` §DR assumes. See [`README.md`](../README.md) for the index.

**Original status (2026-08-17):** draft for review (#790 M.2) — superseded by the banner above. Planning artifact only — no implementation, no branch created, nothing executed. Same discipline as the M.1 spec (merged, #827/#830): everything ruled is made executable; every open input is **marked, with per-option deltas, and picked by nobody here**.
...
- **Run report per attempt** *(none was ever filed here; the rehearsal and the window are recorded in `../../planning/2026-09-16-legacy-tear-out/RUN_LOG.md`)*: `runs/<date>-<leg>-<n>.md` (tracked files, never comments): branch identity block (§2.1 values), per-step wall-clock table, gate outcomes, §3 checklist results, anomalies verbatim.
