...
> **Status (2026-09-02): ABANDONED by owner ruling.** No 3e transform file was ever written and none will be: legacy data is not migrated and the target is greenfield (`00` FC-7 §6, `03` §Post-ratification rulings; first recorded at #1171 §3, confirmed 2026-09-02). The legacy lineage still sits untransformed in the `legacy` schema pending the 3f snapshot and the 3g drop (#1202). The production measurements this spec waited on are on #943, closed as moot (Fork C collapses, D is answered by the schema, E was live at 233 rows, A has no purchase on this corpus). Retained as the record and as the source should a transform ever be written from `legacy` or its snapshot.
