...
- [x] 3b: worker startup log prints `lanes: interactive×4 bulk×2 pool=10` (`assert_concurrency_fits`, at composition — an oversubscribed configuration refuses to start); the K-loop gate (`TestKLoopsOnOneLane`) records `checked_out_peak ≤ K + 1` with K = 3 bulk tasks and no `TimeoutError` (2026-09-11, the 3b PR).
