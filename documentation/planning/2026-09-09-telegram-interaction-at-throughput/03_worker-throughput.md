...
- [x] 3b: a slow send on one binding does not delay a send on another (`TestASlowChatDoesNotDelayAnother`: a 3 s send on one chat, the other binding's row sent within 1.5 s under two interactive tasks); `one_slow_chat` re-run 2026-09-11 (`reports/2026-09-11-rtt10ms.md`, before 3b: answer p95 0.602 s) — the edit-landed bound is read from the next run after 3b lands.
