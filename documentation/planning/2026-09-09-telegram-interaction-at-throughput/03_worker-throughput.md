...
*(State on 2026-09-09, before 3a/3b: the pinned claim connection, `ws_lane_cap = 2`, the flat 60 s backoff and the unbounded mint sweep were all replaced by the steps below — `WorkerConfig` in `src/services/target/work_loop.py` carries the as-built values.)*
...
- [x] 3b: worker startup log prints `lanes: interactive×3 bulk×2 pool=10 (tasks hold up to 7 + 3 reserved)` (`assert_concurrency_fits`, at composition — an oversubscribed configuration refuses to start; a bulk task is weighed at two connections because its plain kinds open their own sessions under the loop's job transaction — the adversarial review's count); the K-loop gate (`TestKLoopsOnOneLane`) records `checked_out_peak` = K exactly (3 bulk tasks) and the sender gate `≤ 3` under two sender tasks, no `TimeoutError` (2026-09-11, the 3b PR). **The reserve, honestly:** `RESERVED_CONNECTIONS = 3` counts the STEADY holders — the pinned election plus a clock tick and a heartbeat beat coinciding; the status reporter, the two sweepers and `reconcile_ambiguous`'s second session are transient readers that wait `pool_timeout` when every task is DB-active at once, and a claim's pool wait is counted (`waits=` on the status line), never an error toward the lane's ceiling (structural review of the 3b PR).
- [x] 3b: a slow send on one binding does not delay a send on another (`TestASlowChatDoesNotDelayAnother`: a 3 s send on one chat, the other binding's row sent within 1.5 s under two interactive tasks); `one_slow_chat` re-run 2026-09-11 before 3b (`reports/2026-09-11-rtt10ms.md`: answer p95 0.602 s) and after (`reports/2026-09-11-rtt10ms-post3b.md`: answer p95 0.537 s, the other chats 0.512 s over 180 taps, `taps_across_many_cards` 0.615 s). **The edit-landed bound is not readable from the harness as configured:** `outcome_landed` is 0 in both 200-tap scenarios, before and after 3b — the 12 s settle window ends before the sender's first supersede lands (the sweep's 3 s cadence plus the per-chat pacing of 18/min against 20 cards per chat) — filed as #1292; the worker-side property is proven on real rows by `TestASlowChatDoesNotDelayAnother`.
- [x] The leased index migration applied with its postcondition (074 / `07` §20, `ix_jobs_leased_lane_ws`).
- [x] `05:31`, `05:33`, `05:38` marked built; #716 answered with a citing comment (2026-09-11, after #1288). (`05:33` and `05:38` by the 3a PR; `05:31` by the 3b PR.)
