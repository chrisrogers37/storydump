...
- **Join, invited path (FC-6 — the app delivers, email or Telegram):** admin+ creates a `workspace_invitations` row (`role='member'` — the invitation ceiling, built to the `03` pass-4 lean; elevation is a post-join role change, below). Delivery follows the row's `delivery_channel`: **email** — a `send_email` job through the `07` §1 port to the invited address; **telegram** — an `invitation` outbox card on the workspace's `telegram_group` binding (`02` §6), carrying the accept button and the `t.me/<bot>?start=inv-<token>` deep link. **Platform constraint, stated:** a Telegram bot cannot initiate a DM to a user who has never started it, so Telegram delivery means the bound group's card and/or the start link — a workspace with zero bindings (web-born, `02` §1) is email-only, and the UI says so. **Accept, either surface:** the invitee authenticates (`07` §1 Google sign-in, or taps as their Telegram identity), presents the token, and passes the per-provider acceptance constraint (`03` D33: Google — the verified id_token email must equal the row's `email` when set, mismatch refuses, match records `accepted_email_matched=true`; Telegram — the tapping identity must match `invited_channel_ref` when set, no hint ⇒ token possession accepts with the skip recorded as `false`); the accept CAS and the member INSERT commit in one transaction (`02` §1). Re-invitation = new row + revoke prior, same transaction. Stale invitations expire via the `reap_expired` sweep (`02` §5 remit — no bespoke reaper).
