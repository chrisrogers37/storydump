...
- **Open (product owner):** PA-1 — implementers build default (a) until ruled; email-provider ack (reopened by FC-6) — Resend as swappable default until acked; Google consent-screen publishing status (console fact); invitation role ceiling — member-only built to lean (`03` pass-4 items).
