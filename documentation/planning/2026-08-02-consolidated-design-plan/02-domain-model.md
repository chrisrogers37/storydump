...
This file is DDL-complete: every table below is stated as executable PostgreSQL, and every invariant claimed by this plan names the database object that enforces it. Prose between DDL blocks explains *why*; the DDL is the decision. Bare R/T/H ids cite `01` §Requirements ledger. Current-state (legacy) column facts were grounded, at pass 4, in the legacy tier's `src/models/*.py` (deleted in #1216), `scripts/migrations/001–050` and `scripts/setup_database.sql` (kept only as the lineage lane's by-hand base) — the `legacy` schema itself was snapshotted to `archive` (078) and dropped (079) — and the per-column legacy mapping each track needs is restated in `04`; no external extraction artifact is required reading.
...
| approved | publishing | worker | **one transaction** (SQL below): key-4 acquisition + atomic cap debit + lease held — **or, since 2026-09-14 (plan 03), re-entry on the debit the story already carries** (the flip is re-entrant; amendment below) |
| approved | expired | reaper | approval stale beyond policy — **seeded, but no longer taken since 076 (plan 03): the reaper parks a stale approved story for review instead** |
| approved | cancelled | worker | `cancel_requested` (pre-publish it is always honorable); **a stepped-back story's debit is refunded in the same transaction and its transit asset destroyed after commit (plan 03)** |
...
| publishing | review_required | worker | poison: attempts exhausted on a retryable class (G5), or a class ladder spent (the float, plan 03); a dead job parks from here too |
| publishing | approved | worker | **the float (076, plan 03):** a wait between attempts — a refused fetch's round spent, a lost container, a rate limit, a pending poll, the publish cap — steps the story back OUT of the account's slot with `publish_step`, `transit_asset_ref` and `cap_consumed_on` intact; audited with its class and rung (`float_wait`); re-entry is the flip above |
| approved | review_required | worker / reaper | **the float's safety nets (076, plan 03):** a dead publish job, or an approved story the pipeline has not finished within the approved TTL, is parked for the workspace's review with the card and one notice — a decision the workspace made is never silently undone |
...
**Amendment (2026-09-12 — the review card is the workspace's to resolve; PR "the review card's three resolutions").** The four `review_required` exits above are typed `user (operator)`. Three of them are now taken by a **workspace member** through the command `resolve_review` (floor `member`, like every other intent command): `retry` → `approved` (the printed debit-neutral transaction below, `publish_cap.resolve_retry`, then the publish job is minted again under `approve`'s own gates), `posted` → `posted` (`publish_cap.resolve_posted`: legal only with `publish_step = 'publish_called'` and a container, `published_via = 'api'`, `publish_step = 'effect_confirmed'`, the debit stands, the post's effects written), `cancel` → `cancelled` (`publish_cap.resolve_cancel`, debit retained). `failed` — the refund — is NOT offered to a member and stays the operator's edge, with `clear_quarantine`, until the operator principal exists (#1124). Why the actor moved: first principles for many tenants — the member is the human who can look at their own story, and an operator-resolved queue cannot scale to thousands of workspaces; the member already holds `approve`, `mark_posted` and `cancel`, and the three resolutions are those levers on a parked row. The card carries the three as buttons (`prompts.review_keyboard`: ✅ It posted · 🔁 Not there — post again · 🚫 Give up, `v1:itposted|notposted|giveup:<intent>`), written by the pipeline's poison and the reconciler's park through the restate-by-ref door (§6: a `prompt_supersede` row may carry a `reply_markup`); the web Queue offers the same three on a `review_required` row (It posted only once a publish call was made — the row's `publish_step`). A resolution reaches the cards the approve tap already superseded by ref, in every binding, in one statement (`outbox.restate_everywhere`). **The publish op is the discriminator, and every resolution ends it** (review of 2026-09-12): the latest `publish` row of `provider_operations` says what Instagram was asked and answered — `ambiguous` (the answer was lost; it MAY have posted), `failed` (it answered no), none (never asked). `retry` on an `ambiguous` op needs the member's verdict `not_posted` (the retry button's label IS that answer; the Queue asks first) and ends the op `failed` by verdict BEFORE re-approving — a plain retry would re-permit a second publish call beside an unresolved one, the permit rail's one forbidden thing — and is refused `may_have_posted` without it; `posted` is refused `nothing_to_confirm` unless a publish call was made and Instagram did not answer no (`publish_step` alone stays `publish_called` after a definitive refusal), and ends the op `succeeded` by verdict; `cancel` ends it `failed` by verdict `given_up`. So no op stays in the un-retirable class after its intent resolves (§6). A plain `cancel` command on a parked row, and `disable_account` for its account, take the give-up edge outright (a parked row has no worker checkpoint to honour a flag at) and the card loses its buttons. (2026-09-12, later the same day: the first-fetch discriminator was removed — a 9004 rides the ladder every time and never locks the file, so a retried intent's fresh asset needs no episode filter.)
...
```
**Amendment (2026-09-14, plan 03 — the float):** the flip is re-entrant. A story that stepped back to `approved` between attempts carries `cap_consumed_on`; the debit CTE selects from the approved row and runs only when it does not, the flip's `cap_consumed_on` is `COALESCE(existing, debit)` (so `ck_publishing_debited` holds), and the flip's own WHERE refuses a `cancel_requested` row. The tuple grows to `(debited, flipped, was_approved, cancelling)`: `(1,1,1)` a fresh debit, `(0,1,1)` re-entry on the debit the story carries, `(0,0,1,false)` the cap denied it (defer to the next slot), `(0,0,1,true)` a cancel landed (the cancel path, refund before the flip), `(0,0,0)` not an approved row (`IntentNotApproved`), `(1,0,·)` raise. Every wait steps the story back through the new `publishing → approved` edge, so the account's slot is held only for the seconds of a call; a busy re-check backs off 20/40/60 s. A story that re-enters on day D+1 keeps `cap_consumed_on = D`: the refund stays symmetric and D+1's count omits it (§8) — the same shape as error 9.

**Amendment (2026-09-12, #1301):** the key-4 refusal is NOT "treated exactly as a cap denial" any more. The flip's CTE runs in a SAVEPOINT — the refusal used to abort the whole admission transaction, so the deferral's own writes crashed and the job burned an attempt on the failure ladder (five approvals in six seconds cost four such crashes) — and the outcome is its own (`FlipOutcome.BUSY`, audit reason `exclusive`): the intent waits `BUSY_RETRY_SECONDS` (20/40/60 s, a ladder) and tries again, because a sibling's publish takes seconds, where a cap denial rightly waits for the next product slot. The debit still rolls back with the savepoint; no error is surfaced.
...
**Amendment (2026-09-09 — the tap, phase 1 of `../../archive/2026-09-09-telegram-interaction-at-throughput/`): read, then decide.** An intent executor reads its row `FOR UPDATE` and a row in ANY state other than `awaiting_approval` *answers* with its current state (`CommandResult("answered", {state, settled_by, settled_at, outcome_text})`) and writes nothing to the intent — a repeat tap, a finished card, the `review_required` edge (the review card's own command `resolve_review` takes it since 2026-09-12), a card not yet prompted. The guard (§4) stays the last line, not the first. Only a row that was `awaiting_approval` at read time flips, and every flip supersedes the intent's cards in **every** binding of the workspace inside the same transaction (a rolled-back flip takes its edits with it). A card whose cancellation is requested is refused as `cancelling`, by its own name. The Telegram tap reaches this port exactly as the web does — the chat resolved through `fn_resolve_binding`, the tapper through `user_identities`, the actor GUCs set on the ingress connection — and the transaction is the acknowledgement (R5); the `answerCallbackQuery` follows the commit, best effort, outside the outbox on purpose (the immediate keyboard strip that stood beside it was retired 2026-09-12: the paced supersede edit removes the keyboard, §6).
...
**Send-state authority is single-homed:** the outbox row IS the delivery record and the only authority on "did this send" — the sender *job* carries execution state only. The `ambiguous` outbox state carries R8's no-blind-retry rule, resolved per kind, because Telegram provides no general read-back for a lost `sendMessage` response (there is no "list my sent messages" API): **`notification`/`ack`** — retry once after backoff, then `failed`; a duplicate notification is the accepted cost, bounded at one. **`approval_prompt`** — resend; two live cards for one intent are tolerable because both resolve to the same intent and terminal-state-first reads (R6) make whichever is tapped later render the terminal state; a card is rendered again at claim time, whether a first send or a resend — a slot that moved on is retired unsent, a live one goes out with the workspace's current buttons and its record rewritten to match (2026-09-12: a lost answer's resend three days later showed the keyboard of the day it was minted); on any intent state change, supersede-all (`prompt_supersede` rows target every known `external_message_ref`; a card whose ref was lost heals on first touch — `adopt_card`, 2026-09-15). **`invitation`** (FC-6 Telegram delivery) — approval-prompt semantics: resend on ambiguity, tolerate a duplicate card, and on ANY terminal transition of the invitation (accepted / revoked / expired) **supersede-all** — `prompt_supersede` rows target every known card ref. The card is addressed to the workspace's `telegram_group` binding, carries the accept button (callback → ingress inline transaction, R5) and the `t.me/<bot>?start=inv-<token>` deep link (`07` §2's start-token door), and is rendered and delivered by the existing sender under the same `tg:<binding_id>` serialization — zero new senders, zero parallel delivery paths. **Edits always go supersede-then-send** — never edit-in-place on an ambiguous ref. This is the complete ambiguity policy; there is no "stamp-heal" beyond the ref capture on a late Telegram response. **Amendment (2026-09-09, the tap; re-shaped 2026-09-12):** a `prompt_supersede` row *edits the message it names* in ONE call — the card's original header plus the outcome line as a caption or as text by the `sent_as` `mark_sent` recorded on the sent row's payload, with the keyboard the state offers in the same request — empty for every settled card, the review card's three buttons for `review_required` (2026-09-12: the `prompt_supersede` row carries a `reply_markup`) — ("message is not modified" is success); a row without an outcome, or a combined edit Telegram refuses, falls back to `editMessageReplyMarkup` with that same keyboard alone (type-agnostic; the call that must land — a refusal of it escapes to the sender and settles like any lost send). One Telegram message per tap per binding: the bot's 30 messages/s is the fleet's ceiling, and the route's separate unpaced strip is retired. Two consequences (#1297 review): a `prompt_supersede` whose answer is lost is **resent** under `MAX_PROMPT_RESENDS` — and the policy is now actually applied to a live sender's `ambiguous` rows: the binding's next claim resolves those that have waited `AMBIGUOUS_RESOLVE_AFTER_SECONDS` (30 s), where before only a dead sender's stranded rows reached `resolve_ambiguous` — and the sender sweep mints a sender for an aged ambiguous row as it does for a pending one, so a quiet binding does not wait for its next card; a lost edit whose message has a NEWER `prompt_supersede` (the pipeline's "✅ Posted" after the tap's "✅ Approved") is retired `superseded`, never resent, because a resend carries its own older line; a definitive 4xx refusal (`ChannelRefused`) fails the row outright, like a gone destination — the edit is idempotent, so a duplicate costs one paced call and nothing else, where the notification's single retry would leave a card live with buttons after one bad Telegram minute; and an orphan card whose ref was never captured (the first copy of a resent upload) keeps its buttons until touched — a tap adopts it (`adopt_card`, 2026-09-15) — the route's strip was the only path that reached it. `supersede_all` targets `sending` rows too, because **the sender commits before it speaks**: the poller's tick is transaction-per-checkpoint (pace and claim, commit, provider call with no transaction open, settle) — so no rate row is locked across a send (#1260) and a tap never waits on an upload; a sender whose `mark_sent` is fenced by a supersede in flight queues the edit for the ref only it received, and after every sent `approval_prompt` it re-reads the intent (R6) and edits a card whose intent moved meanwhile. Cards end in **every** terminal state: a settled-card sweep on the reaper's beat supersedes the live cards of any intent already `posted|skipped|rejected|expired|failed|cancelled` with the terminal line. **Amendment (2026-09-10, phase 3a of the tap plan):** a provider **429** is a third answer beside "sent" and "lost" — the message did not go and the row is not at fault — so the sender returns the row to `pending` and writes the provider's `retry_after` as a **durable hold** on the pacing rows — for a chat-addressed call (almost always the chat's ~20/min limit) the chat's row for the whole `retry_after` and the `tg_global` row for a 2 s brake; for a call with no chat, `tg_global` for at most 60 s — in one statement, holds before the row's own state change (a row superseded in flight keeps the holds), so every replica's sender defers on the rows until it passes; the sender job reschedules itself for that instant with its attempt restored and never sleeps in-task. The `approval_prompt`/`invitation`/`prompt_supersede` resend is **bounded at three** (`MAX_PROMPT_RESENDS`): past it the row is `failed` and the intent's reaper ends the card — the unbounded resend was a card that could be re-sent forever. **Since 2026-09-15 a twin whose ref was never captured heals on first touch:** the tap carries the message id Telegram named, and `outbox.adopt_card` makes it a known card of the story — superseded, edited with the story's line at once, reached by every later line — provided the story has a card on that binding and no row on the binding already owns the ref (a callback's intent token is not bound to its message id by Telegram).

**The reconciliation contract (ambiguous IG publish — evidence, R8).** Pass 3 stopped assuming the platform fact 0.4 exists to verify: whether an Instagram container exposes an observable **post-publish** terminal status at all. **0.4 verified this on 2026-08-13 and the uncertainty is closed — the premise was wrong.** Meta's *Instagram* content-publishing guide documents `PUBLISHED — The container's media object has been published` among the five `status_code` values, alongside `EXPIRED`, `ERROR`, `FINISHED` and `IN_PROGRESS`. It is not a Threads-only value, so the pass-2 contract did **not** borrow its evidence from the wrong API (this resolves R3 review §6.11, which was marked uncertain rather than false — correctly, as it turns out). A post-publish observable terminal status exists, the authoritative-positive set is non-empty, and the seam below resolves to **container-verdict mode**. Citation and verbatim quotes: `0.4-meta-primary-doc-verification.md`. **One contract, parameterized by evidence authority:** the config seam `reconciler_evidence_mode` records 0.4's doc-cited verdict as the sets of container `status_code` values that are **authoritative after `publish_called`** — which value (if any) is authoritative-positive (the effect happened) and which (if any) are authoritative-negative (it definitively did not). Setting the seam is a config change, never a design change. The machinery is mode-independent:
...
- The two 0.4 outcomes are just the seam's values, named for `04`'s gates: **container-verdict mode** (a post-publish-observable value is confirmed — the authoritative-positive set is non-empty, and most ambiguities terminalize mechanically) and **evidence-capture mode** (0.4 finds none — both authoritative sets are empty, so the machine never self-terminalizes and every exhausted ladder parks `review_required` with the captured trail). `04` L.3's gate carries a test per mode. **0.4 selected container-verdict mode (2026-08-13).** Both modes stay implemented — the seam is a config value, and evidence-capture remains the correct behaviour for any account or API version where the positive value stops arriving.
- **Seam values as set by 0.4.** Authoritative-**positive**: `PUBLISHED`. Authoritative-**negative**: `EXPIRED` (Meta: "the container was not published within 24 hours and has expired") and `ERROR` ("the container failed to complete the publishing process"). Non-terminal: `IN_PROGRESS`, and — **the trap worth naming** — `FINISHED`. After `publish_called`, `FINISHED` means *ready to be published*, which is evidence the publish has **not** landed, not evidence that it failed; classifying it authoritative-negative would terminalize a still-live publish as a definite non-event. Meta also recommends "querying a container's status once per minute, for no more than 5 minutes" — the `05` ladder is more conservative than that from the second rung on, so it does not conflict, but the long tail out to container expiry is our extension rather than Meta's advice.
...
- The endpoint semantics (container `status_code` vocabulary **and which values, if any, are authoritative post-publish**, stories lookback validity) were 0.4 primary-doc verification items — platform inputs under `05`'s revision rule — and 0.4 is a named prerequisite of L.3/L.5 (`04`). **Verified 2026-08-13; both are settled above and below.**
- **The exhaustion tail's stories check is corroborating, not dispositive.** 0.4 confirmed the 24 h lookback ("stories are only available for 24 hours") and surfaced two exclusions not previously recorded: responses **omit Live Video stories**, and **a story created by resharing is not returned**. So a story absent from the list is not proof it was never published, and the final check must not be read as a negative verdict on its own. Container-verdict mode makes this far less load-bearing than it would have been under evidence-capture — the tail now runs mostly for intents the ladder could not terminalize — but the tail still runs.
...
`service_runs` (legacy ops bookkeeping) was snapshotted to `archive` (078) and dropped with `legacy` (079); the planned M.1 attribution transform never ran, and it never lands in the target (pass 5/FC-7; `03`).
...
| `fn_reaper_stale_approved(ttl interval, lim int)` (`svc_maintenance`) | the reap executor's approved leg (plan 03 of the first-fetch investigation, 076, `07` §22): returns the `approved` stories past the TTL with their workspace ids, paused workspaces excluded — a cross-tenant **read** in the `fn_reconciler_sweep` shape; the executor then asserts each row's tenant and parks it for review (or refunds and cancels a `cancel_requested` one) as `svc_worker` | `p_lim` TOTAL, the sweep's remainder | `svc_worker` |
| `fn_retention_batch(class text)` (`svc_maintenance`) | one `05` retention class per call: age-qualified DELETE walking its `ix_*_retire` index; the `audit_events` class COPY-exports into the archive schema first and **aborts if the export fails** (07 §4) | `p_batch` per class per call (`05`) | `svc_worker` (granted; **no executor calls it yet**) |
...
| `fn_auth_plane_sweep()` (`svc_maintenance`) | expiry/retention deletes on `oauth_states`, `session_tokens`, `command_dedup`. `svc_maintenance` carries three enumerated auth-plane `USING (true)` policies + DELETEs **for exactly this door** — resolving the pass-2 contradiction (R3 §6.7, maintenance held DELETEs its policies could never see) by adding the missing policies to the NOLOGIN sweep owner, never by handing a login the DELETEs (a `svc_ingress`-owned definer body would have put auth-plane DELETE privileges on the internet-exposed login — falsifying the grant matrix below) | `p_batch` per class (`05`) | `svc_worker` (granted; **no executor calls it yet** — the reaper schedule was to drive it) |
| `fn_memberships_for_caller()` (`svc_membership`) | the user-plane read behind `/me` and `GET /api/v1/workspaces` (pass 8, #1037): the calling user's memberships as `(workspace_id, name, state, role)`, the caller read from `app.actor_user_id` internally — never a parameter — so an unset GUC returns no rows; exists because `p_tenant` on `workspace_members` cannot answer a cross-tenant question for one user, and both lanes refused rather than answered `[]` until it did | single subject — the caller's own rows | `svc_ingress` |
| `fn_resolve_binding(channel, external_ref)` (`svc_membership`) | the chat-inbound resolver (pass 9, #854 (a) / #1242): the one `channel_bindings` row for a named chat, any state, so a caller can tell revoked from never-bound — the pre-context read `p_tenant` denies `svc_ingress` | structural — one row by unique key | `svc_ingress` |
| `fn_group_member_seen(channel, external_ref, user)` (`svc_membership`) | the `06` Telegram join path (pass 9, #1242): a caller-proven person seen in a bound, active group is inserted at `member` (`ON CONFLICT DO NOTHING` — never a downgrade); unbound/revoked chats, inactive workspaces, disabled or vanished users are named outcomes, never writes | structural — one membership row per call | `svc_ingress` |
| `fn_member_remove(workspace, user, by_user)` (`svc_membership`) | the revoke for every join edge (pass 9, #1242): an admin removes a member explicitly; the owner and oneself are refused by name, a non-member is `not_found`. The one DELETE on `workspace_members` in the system — the 057 rule that no login role deletes is why this is a door | structural — one membership row per call | `svc_ingress` |
...
**What changed in the door inventory (pass 5):** `fn_comparator_run` is deleted (FC-7 — shadow-read died with the six-stage machine), `fn_invitation_accept` is added (R4 finding 7), and `svc_membership` joins the NOLOGIN owners — net inventory stays nine doors, all nine now printed above. **Pass 8 (#1037):** `fn_memberships_for_caller` is added as the tenth — printed in `07` §10 rather than inside the content-addressed block above, because arm (b) appends and never amends — bringing the inventory to ten. The inventory table earlier in §7 remains the reviewer's enumeration; this section is its implementation, and the two are held in lockstep by the `advertised_ddl_replay` fixture (a door in one and not the other fails the replay or the review, whichever comes first). **Pass 9 (#1242, `07` §14):** `fn_resolve_binding`, `fn_group_member_seen` and `fn_member_remove` are added under `svc_membership` — the inventory stands at thirteen. **Plan 03 (076, `07` §22):** `fn_reaper_stale_approved` is added under `svc_maintenance`, the listing read behind the reap executor's approved leg — the inventory stands at fourteen.
...
| Legacy table (dropped 079) | Target disposition |
|---|---|
...
| `waitlist_signups` | **out of scope, permanently**: owned by the landing site's Drizzle ORM; no Python migration may touch it (existing repo rule, carried forward; it stays in `public` — `scripts/migrations/NOTE_waitlist_table.md`) |
