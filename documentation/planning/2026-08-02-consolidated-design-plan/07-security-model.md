...
- **Missing-key failure:** a payload no ring entry decrypts (key removed too early, restored backup from before rotation) fails closed — the credential flips `state='expired'`, the account flips `reauth_required`, the re-auth path recovers it. Never guess, never log ciphertext. A ring that cannot be BUILT — no key, or a key that is not a Fernet key — is not this case and never takes this path: it is the process's configuration (`oauth_states.RingUnavailable`), both roots refuse to boot on it, and every decrypt door builds the ring before the `try` that classifies a row (#1401, 2026-09-25).
...
- The CLI (service-routed per X.2 — never direct DB) authenticates with a bearer API token (`sdt_…`, §1/§23); `operator` may run the command route and `readonly` only the read views (`src/api/principal.py` `TOKEN_ROUTES`); every use stamps `last_used_at`; a person-bound token's writes audit as that person, `channel='cli'` (superseded in detail by §23).
- Issuance is a signed-in session's act (Settings › API tokens; `POST /me/tokens` for a person-bound token, `POST /workspaces/{ws}/tokens` at the admin floor for a workspace service identity — `src/api/routes/tokens.py`); revocation by the owner of the token or an admin of its workspace (`DELETE …/tokens/{id}`, also `storydump tokens revoke`). No break-glass INSERT exists (superseded by §23).
...
| MultiFernet ring + reencrypt job | L.6 (ring), runbook standing; job kind exists from L.2 (kind only — the executor is unbuilt, `work_loop.UNBUILT_KINDS`) |
| audit grants + GUC-required trigger | L.1 (ledger create) |
| retention sweep + archive export | S.4 (with `05` retention table) — unbuilt as of 2026-09-20 |
...
### §14. The resolver door, the group-membership door and the member-removal door — the eleventh to thirteenth `02` §7 doors (068, #854, #1242)

`01` §1 assigns chat-inbound resolution to the ingress adapter, and the printed
policies could not do it: `p_tenant` on `channel_bindings` is
`workspace_id = app.tenant_id`, so with no tenant claimed a bound chat reads
as never bound — measured, and pinned fail-closed, in #854. Its lean (a) is
built here: `fn_resolve_binding` exposes exactly one row for a named chat,
where a lookup policy would expose every workspace's topology.

The `06` Telegram join path rides on it: `fn_group_member_seen` makes a person
seen in a bound, active group a `member` — never a downgrade (an owner or admin
stays what they are), never a write for an unbound or revoked chat, an inactive
workspace or a user that vanished mid-flight (a disabled user is refused by the
caller, which can read `users` — the door holds no grant on it). **The caller
proves the person**: `p_user` is the ingress adapter's resolution of the
sender's linked Telegram identity (`user_identities` is role-open to it), and
the door trusts that the way the worker trusts `job.workspace_id` — stated
here, not re-checked in-body. The actor GUCs are set inside the door
(transaction-local, so it is the last governance write in its transaction) and
the row is attributed to the joiner as `user`, the convention
`fn_invitation_accept` set; `svc_ingress` holds no DELETE on `workspace_members`
(057: no login deletes; 068's postcondition pins it) — the door is the one deleter. **Only people the bot can see count**: under Telegram's default
privacy mode a bot in a group receives commands, replies, mentions and service
messages, so the adapter observes speakers *and* the people a `new_chat_members`
service message names, and the runbook names the precondition (privacy mode
off via BotFather, or the bot a group admin). Leaving a group removes nothing
(`06`): the revoke is `remove_member`, built beside this path, and the Members
card names who joined from Telegram. **That revoke is the thirteenth door**,
`fn_member_remove`: the 057 grant matrix is structural that no login role
deletes — a delete lives only inside a door body as its NOLOGIN owner — so
`svc_membership` receives the one `DELETE ON workspace_members` in the system
here, backing exactly that body. The caller proves the admin (the command port
holds `remove_member` to the admin floor); the door refuses by name what no
admin may do (the owner; themselves) and the audit row names the admin, whose
unit of work set the GUCs.
...
### §25. The worker's doors: the tenant-less sweeps read the estate through svc_maintenance (082, #751 part 2)

**Why:** the worker's outbox sender sweep, prompt sweep, settled-card sweep, stranded-source alert
and the reconciler's container poll and ladder count opened a session with an empty tenant and the
`system` actor and ran SQL on policy-covered tables — the class §24 closed for the API's health
surfaces. Under `svc_worker` the sender sweep mints nothing (no card is ever delivered), the prompt
sweep finds nothing (nothing ever asks for approval), no ended story's card loses its buttons, the
ladder counts every step as the first and never parks a lost answer for review, the alert is blind
and the poll inconclusive forever; the writes fail `p_tenant`'s `WITH CHECK`. Five SECURITY
DEFINER doors carry the READS: the sender sweep as one door, because its single `INSERT … SELECT`
with the `NOT EXISTS` live-job check is its idempotence; the due stories, the pending prompts, the
settled cards and the stranded sources as reads that return each row with its workspace, so the
prompting, the transitions, the supersede and the `alerted_at` stamp run per workspace under that
workspace's tenant and the ledger's triggers attribute them as they do a member's own. A sweep
hands its caller's scope back when it is done (`unit_of_work.WorkspaceClaims`): `plan_slot` runs
the prompt sweep inside a tenant job's transaction and then finalizes that job, which the policies
...
either its attempts or its deadline. #1361 found the two Python producers that hand-wrote their
INSERT and so minted jobs bounded by attempts alone; `fn_clock_tick` is the third and last, and it
mints five kinds that way — the recurring system singletons, due account slots, due credential
refreshes, due source syncs and reauth prompts.

**The deadline is measured from `run_at`, not from `now()`**, and that is the whole reason this is a
considered replace rather than a column added in five places. #1361 paid for the distinction at the
offboarding site: a job minted long before it is due, carrying a deadline anchored at mint, is
already spent when it first becomes claimable and dies on its first run. Four of these five legs
mint at `now()`, where the two instants coincide; the first does not, because a singleton's
`run_at` is its last completion plus its cadence.

**A deadline helps the singletons rather than harming them.** Leg 1 mints only when no ready or
leased row holds the kind's key, so a job stuck `ready` blocks its own successor for ever — attempts
never advance, because nothing runs it. The deadline is what lets the reaper end it and the next
tick re-mint. For legs 2-5 the cursor is advanced at mint, so a job that exhausts its bound is a
missed beat, as it already is when its attempts run out.

The attempt literals are carried forward unchanged. Three legs write 3 and two write 5 while the
bulk lane's budget is 5; unifying them changes how often a failing job is retried, which is a
behaviour change of its own and is recorded in #1381 rather than taken here.

```sql
-- [§26 the clock's five legs mint with a deadline]
