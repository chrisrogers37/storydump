...
1. **Local media is dropped.** No local-filesystem media path exists in the target — `main`'s `local` default and its upload-item class have no target counterpart. **The production zero-row gate is retained as a window precondition despite the clear ruling** (a wrong assumption about production contents silently loses the owner's media); `04` M.1 is the gate's normative home (both counts, halt-and-route semantics, resolutions).
