...
> **Written 2026-08-02 against the legacy tier; read with this map.** The target tier shipped greenfield (worker 2026-08-24, API 2026-08-31) and the legacy tier was deleted on 2026-09-17 (#1216). The design below is the ratified record; where a paragraph still says "today" or "Mini-App" it describes the tier that is gone. **Where each role lives as built (2026-09-20):** ingress = `src/api/app.py` with `src/api/routes/{auth,v1,tokens,ops,webhooks,meta}.py`, the Telegram transport `src/channels/telegram_transport.py` and the tap door `src/services/target/webhook_ingress.py` → `telegram_dispatch.py`; the command port = `src/services/target/{vocabulary,commands,command_executors}.py`; workers = `src/worker.py` (`WorkerApp`, `SenderSweeper`, `PromptSweeper`) over `src/services/target/work_loop.py` (the executor registry; `UNBUILT_KINDS` names what is registered but not built); the clock = `src/services/target/scheduler.py` (`ClockElection`) driving `fn_clock_tick`; the publish pipeline = `publish_pipeline.py` + `publish_cap.py` + `provider_ops.py` + `transit.py`; the outbox sender = `outbox.py`; the egress floor = `egress.py`; the unit of work and the tenancy GUCs = `unit_of_work.py`. Not running as of 2026-09-20: `retention_sweep` and `reencrypt_credentials` (no executor), the auth-plane sweep door (never called), the Meta usage pre-check (behind `TARGET_USAGE_PRECHECK_ENABLED`, off).

## Requirements ledger (normative — the R/T/H ids cited throughout this plan)

Derived from the product and platform, validated by cross-check and #730. `RF-Rn`/`RF-Gn` elsewhere in this plan cite `documentation/archive/2026-07-29-high-throughput-multi-tenant/review-findings.md` (#730, archived); bare `Rn`/`Tn`/`Hn` cite this ledger.
...
**3. Scheduler-as-clock (tiny singleton, pg-advisory-lock elected from among the workers).** Converts time into jobs, nothing else: one indexed scan per tick over precomputed `next_*_at` columns → idempotent job inserts (H3). Every heavy step is a worker job, so a tick is milliseconds and never slips. The **dispatcher** (due-scan → job insert) is `fn_clock_tick` (059, re-created 062/063), driven by the elected clock in `src/worker.py` (`04` L.7 — a step #722's sequence never built at all).

## Interaction-layer port (FC-2)

The domain is already **state-centric** at its heart — an approval is a database state, not a Telegram card (the legacy `posting_queue.status`, gone with the `legacy` schema in 079; now `post_intents`). What it was *not* yet, on the legacy tier, is channel-neutral: the legacy queue row carried `telegram_message_id`/`telegram_chat_id` columns and a delivered-requires-message-id constraint (pass-4 anchor). The port makes the neutrality explicit and strips the channel columns off domain state:

- **Inbound:** adapters normalize to the **closed command vocabulary** — this list is its normative home, and adding a command is a deliberate change here, ratchet-visible: `approve` · `skip` · `reject` · `mark_posted` (manual mode) · `cancel` · `autopost_now` · `sync_now` · `settings_change` / `account_settings_change` · `pause_workspace` / `resume_workspace` · `connect_account` / `reconnect_account` / `disconnect_account` · `move_account` / `disable_account` · `create_workspace` / `rename_workspace` / `offboard_workspace` / `restore_workspace` · `invite_member` / `remove_member` / `change_role` / `transfer_ownership` · `resolve_review` (posted|retry|cancel by the workspace member, 2026-09-12; the failed resolution is reserved for the operator) · `clear_quarantine`. Each carries resolved domain ids only; core services never see a chat id or callback payload.
- **Outbound:** domain code emits **interaction requests** (approval-request, notification, digest) addressed to a `workspace_id` + audience; rows land in `channel_outbox` per push binding. A Telegram approval card and the web Queue (`landing/`, reading the workspace's intents through `src/api/routes/v1.py`) are two renderings of the same `awaiting_approval` intent — the web surface reads the same state via API (pull, no outbox rows); a tap and a click converge on the same command.
...
- **Enforcement:** the FC-2 ratchet (installed `04` F.6) — including its structural rule that no module outside the adapter allow-list (`scripts/telegram_ratchet_baseline.json` — four modules) and no domain model imports a Telegram library or holds a Telegram-typed column (`src/services/core/` was deleted in #1216; the segment reads empty). One mechanism; no parallel grep gates.
...
No broker/Celery/second datastore in the core; no per-workspace bot tokens, processes, or env vars (T3); no polling-loop hierarchy (the legacy background loops collapse into clock → jobs → workers + webhook — done: those loops were deleted in #1216); no global mutable singletons (breakers, sync gates, class-var counters — all cross-request state is rows); no fixed **short-cadence per-source** Drive polling as the freshness mechanism (H4's demand-driven shape + slow jittered baseline replaces it); no per-user Cloudinary credentials (FC-3); no Facebook Page auth (FC-4); no local-filesystem media path (FC-8 — full cloud; sources speak only through the media-source port above).
