...
- **T2** Performance isolation and fairness: one workspace's bulk load, failing token, or provider outage never delays another's post or interaction; fault quarantine per (workspace, provider), never global; no structural starvation.
...
The domain is already channel-neutral at its heart — an approval is a state on `post_intents`, not a Telegram card. The port makes that explicit:

- **Inbound:** adapters normalize to a closed command vocabulary (approve / skip / reject / autopost / sync-now / settings-change / connect-account / …), each carrying resolved domain ids only. Core services never see a chat id or callback payload.
