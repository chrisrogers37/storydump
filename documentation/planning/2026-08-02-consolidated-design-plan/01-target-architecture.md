...
- **Inbound:** adapters normalize to the **closed command vocabulary** — this list is its normative home, and adding a command is a deliberate change here, ratchet-visible: `approve` · `skip` · `reject` · `mark_posted` (manual mode) · `cancel` · `autopost_now` · `sync_now` · `settings_change` · `pause_workspace` / `resume_workspace` · `connect_account` / `reconnect_account` / `disconnect_account` · `move_account` · `create_workspace` / `rename_workspace` / `offboard_workspace` / `restore_workspace` · `invite_member` / `remove_member` / `change_role` / `transfer_ownership` · `resolve_review` (posted|failed|retry|cancel) · `clear_quarantine`. Each carries resolved domain ids only; core services never see a chat id or callback payload.
