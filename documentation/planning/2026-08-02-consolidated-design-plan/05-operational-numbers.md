...
| Approved TTL (an `approved` intent the pipeline never took) | 4,320 min (72 h) — **provisional**: the worker's `approved_ttl_seconds` default since the first slice, no `02` number behind it | reaper clock (`02` §4) |
