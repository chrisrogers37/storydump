...
|---|---|---|---|---|
| 1 | Per-process concurrency (pool slots/replica) | 10 | 50 | interactive: peak 12/s × 2 s ≈ 24 concurrent ÷ 3 replicas = 8, +~25% margin → 10. bulk: 65 steady × ~2.3 headroom ≈ 150 ÷ 3 replicas = 50. **Built (2026-09-11, tap plan phase 3b) as K claim-and-run TASKS per lane on pooled checkouts, not pinned connections: `WorkerConfig.lane_concurrency` interactive 4 / bulk 2 (`TARGET_WORKER_INTERACTIVE_CONCURRENCY` / `TARGET_WORKER_BULK_CONCURRENCY`), under the composition-time ceiling `K_interactive + K_bulk + 3 ≤ POOL_SIZE_SEAM` (10); the three waiting kinds (`deliver_outbox`, `publish_pipeline`, `send_email`) run with no job transaction open. Raising K toward 10/50 needs the pool measurement the gate records (`checked_out_peak`).** |
