...
| Max accepted media file size | image 10 MB / video 50 MB | as built: `MEDIA_CARD_MAX_BYTES`, the Drive adapter's per-card cap (`google_drive_adapter.py`); the 100 MB seam was the design value |
...
|---|---|---|---|---|
| 1 | Per-process concurrency (pool slots/replica) | 10 | 50 | interactive: peak 12/s × 2 s ≈ 24 concurrent ÷ 3 replicas = 8, +~25% margin → 10. bulk: 65 steady × ~2.3 headroom ≈ 150 ÷ 3 replicas = 50. **Built (2026-09-11, tap plan phase 3b) as K claim-and-run TASKS per lane on pooled checkouts, not pinned connections: `WorkerConfig.lane_concurrency` interactive 3 / bulk 2 (`TARGET_WORKER_INTERACTIVE_CONCURRENCY` / `TARGET_WORKER_BULK_CONCURRENCY`), under the composition-time ceiling `K_interactive × 1 + K_bulk × 2 + 3 ≤ POOL_SIZE_SEAM` (10) — a bulk task's plain kinds open their own sessions under the loop's job transaction and so hold two at their peak; the three waiting kinds (`deliver_outbox`, `publish_pipeline`, `send_email`) run with no job transaction open. Raising K toward 10/50 needs the pool measurement the gate records (`checked_out_peak`).** |
| 2 | Global concurrency (deployment-wide) | 30 | 150 | replicas × per-process; headroom framing above |
| 3 | Per-workspace concurrency | 5 | 3 | bulk: 1 publish + 1 sync + 1 misc — a workspace can never monopolize a lane. interactive: capped at half one replica's interactive pool. **Built (2026-09-10, tap plan phase 3a): `WorkerConfig.ws_lane_cap_interactive = 5`, `ws_lane_cap_bulk = 3`, passed to `fn_claim_job` per lane (was 2/2).** |
...
| 6 | Lease duration | 90 s (one value, both lanes — `WorkerConfig.lease_seconds`; the 60/120 band was the derivation) | 90 s | ≥ longest single checkpointed step (container poll segments ≤ 60 s) + margin; steps checkpoint, so a lease never covers a whole pipeline |
| 7 | Heartbeat interval | 20 s (both lanes — `WorkerConfig.heartbeat_interval_seconds`) | 20 s | 3–4 beats per lease; the beat task is independent of pipeline awaits (`02` §5), so provider waits never starve it; missing 2 beats ⇒ presumed dead well inside the lease |
| 8 | Attempt / deadline budget | 3 attempts, deadline +10 min | 5 attempts, backoff 1/5/15/60 min, deadline = slot end (or +6 h for non-slot jobs) | R8: retryable classes only; ambiguous never re-attempts (reconciler owns it). **Built (2026-09-10, phase 3a): `jobs.LANE_BUDGETS` (interactive 3 / +10 min, bulk 5 / +6 h) written at mint as `max_attempts` and `deadline_at`; the ladder is 10/30/60 s and 60/300/900/3600 s, ± 20 % jitter; `_run_job` ends a job `failed` past either bound and a tenant kind the sweeps do not re-mint gets a user-language notification. The slot-end deadline for a publish job stays the pipeline's own ceiling. The float (2026-09-14, plan 03): a fetch wait (30/60/120/300/600/900 s, six) and a container-gone wait (10/30/60 s, three) RESTORE the job's attempt — their bound is their own count on the story (`attempts_by_step.fetch_waits`, `container_gone`); three fresh urls per round before a wait; a busy re-check backs off 20/40/60 s; every wait steps the story back to `approved`.** |
...
| Telegram pacing | 20 msgs/min/group, 30/s global (built with headroom: 18/min/chat and 25/s global — `WorkerConfig.chat_limit` / `global_limit`) | Telegram published budgets, carried into durable `rate_counters` rows (`02` §6 — scopes `tg_chat`/`tg_global`) |
| Jobs-ready poll (workers) | 1 s every lane (`WorkerConfig.claim_idle_seconds`) | the pg-polling cost the annex trigger watches |
...
| Media transfers per worker | n/a — no local transfer stage exists under FC-8; the bound is the bulk lane's K (2) × the egress byte cap (`egress.py`) | EP:80's initial cap, superseded by FC-8 |
| Temp storage | n/a under FC-8 (no local media path; the build's `mkdir -p /tmp/media` was residue, removed by #1394) | SE:236-237 inequality, moot |
| Sync baseline | every 6 h jittered; pre-slot sync at T−15 min if source stale > 30 min (not built — `sync_now` is the only demand producer; the 6 h baseline and chunks of 200 are, `media_sync.py`); first-ingest chunks of 200 files | H4 demand-driven shape |
...
| Cloudinary (FC-3) | **transit TTL = the asset's lifetime**: reap-on-success (minutes, FC-3.5) + hard TTL 48 h + reap sweep every 6 h as built (`WorkerConfig.transit_reap_older_than_seconds`; `reap_transit_assets` recurring in `worker.py`; FC-3.6 said 24 h / 15 min). Delivery URLs are **signed, non-expiring** (FC-3.2 as amended) | per D38 (the analysis's home). **Revisit trigger: sustained monthly Cloudinary credit consumption > 225 credits** (D38's condition; this row is the number's home) |
| Meta usage pre-check | inline-only at publish admission, **behind a default-off flag (the S.5 canary decides)** — wired 2026-09-12 as `TARGET_USAGE_PRECHECK_ENABLED` on the worker (truthy: `1`/`true`/`yes`/`on`; anything else leaves the seam None; a dry run never reads usage; an answer without a quota total proceeds, uncached); in-process cache TTL 5 min keyed on `provider_account_ref` | `02` §8 — **no background refresh exists**; worst case ≤ 1 query per publish attempt (≤ ~520/min at the corrected absolute ceiling; far less at cadence-realistic load), vs the struck eager reading's 1,500/min at 7,500 accounts (review B§6) |
| Parked-intent alarm | `publishing_ambiguous` or `review_required` > 15 min pages; customer notification per `06` §5 after 6 h as built (`WorkerConfig.reconcile_notify_after_seconds`; the design said 24 h) | observability floor (`01`) |
...
| Quarantine backoff ladder | 1 m / 5 m / 30 m / 2 h / 24 h (cap); strike decay 24 h; re-alert dedup 1 h (unbuilt: nothing writes `provider_quarantine` yet) | `02` §2 semantics |
...
| Approval TTL default (`approval_ttl_minutes` NULL) | 1,440 min (24 h) | workspace seam; reaper clock (`02` §4); the worker's `WorkerConfig.approval_ttl_seconds` default, pinned to this row by `tests/src/test_worker.py` |
| Approved TTL (an `approved` story the pipeline has not finished) | 4,320 min (72 h) — **provisional**: the worker's `approved_ttl_seconds` default since the first slice, no `02` number behind it. **Since 076 (plan 03) the reaper parks it for the workspace's review instead of expiring it; a floating story re-enters `approved` at every hop, so its clock restarts and it never reaches the TTL; a paused workspace's stories are not listed; a stale row flagged `cancel_requested` is refunded and cancelled** | reap executor's Python leg (`02` §4), listing through `fn_reaper_stale_approved` (`07` §22) |
| Reaper cadence + budget (`fn_reaper_sweep p_lim`) | built as every 6 h, 200 per sweep (`reap_expired` recurring in `worker.py`, `WorkerConfig.reap_limit`) — the design value was every 60 s, 500, and a lapsed lease therefore holds its serialization key for up to 6 h (the tear-out's owner queue records the gap; #1329); the design's 500 — the sweep's TOTAL, enforced by construction per the `02` §7 door bound rule (pass 7 — R6: this row did not exist and the legs drew p_lim each); priority and fill: the door's own comment is the one home | same bound family as the tick's per-tick 500 |
| Offboarding | grace window 30 days; publish-drain timeout 15 min; revocation retry = the bulk lane budget as built (5 attempts, 1/5/15/60 min ladder — `jobs.py`; the design said 3 × 1 h) | `06` §1 workflow |
...
| Email delivery (`send_email`, `07` §1) | 3 attempts, backoff 1/5/15 min; **provider-wide budget 90/day** (`rate_counters` scope `email_global`, key `''` — headroom under the free tier's 100/day hard pause; over budget ⇒ the job defers on its retry schedule) | provider + bounce semantics: `07` §1 (ack status: `03` items); the volume model is the envelope input above; **X.3's gate no longer delivers a real invitation email — that conjunct is deferred out (`04`), and these numbers apply whenever a provider is chosen** |
| Retention sweep cadence | daily, batches of 5,000 per class, walking the `ix_*_retire` indexes (unbuilt — S.4 open, #1173 / #1327: `fn_retention_batch` has no caller; nothing enforces these windows today, including the 90-day drop of the 078 snapshots, #1326) | `02` §5 pattern; H5-bounded |
...
## Retention (swept by `retention_sweep`, S.4; per-class, terminal/age-qualified — unbuilt as of 2026-09-20, #1173)
...
| PITR floor | Neon PITR window ≥ 7 days — measured 2026-09-18 at 24 h (`history_retention_seconds` = 86400), below this floor; owner decision pending (#1328): raise it in Neon, or amend this row to the measured value |
| RPO | Neon continuous WAL (~minutes) — no additional mechanism |
| RTO target | 1 h (restore branch + repoint DATABASE_URL + smoke suite) |
| Restore drill | quarterly, runbook'd: PITR branch → runner parity check → smoke suite; **the M.2 rehearsal was to be the first drill** (pass 5 — it runs exactly this sequence on a production PITR branch); M.2 never ran, and the first drills in practice were the tear-out's rehearsals of 078 and 079/080 on Neon PITR branches (2026-09-18/19) — the quarterly cadence starts from there. The `archive` schema is covered by construction (it IS the database); S.4's gate additionally proves one audit export and one snapshot restore |
