...
| SD-M2-3 | `svc_migration` rehearsal password via `ALTER ROLE` from the owner session, post-bootstrap (P5). Not outage-shaped, unlike P1.b — **provided no production counterpart exists**. That is an **inference from program state, not a measurement**: no window has ever opened, and the step-0 bootstrap is the only printed creator of `svc_*` roles (consistent with #787's measured no-first-contact world — which enumerated **schemas and tables, not roles**). The inference is **verified before P5 ever runs**, as the one extra SELECT in P1.b's first-contact assertion (zero `svc\_%` rows in `pg_roles`); nonzero rows halt the leg | Neon API-managed role credential — deliberately NOT used for `svc_migration`: routing the runner's login through the managed layer would blur exactly the layer §3 is measuring |
