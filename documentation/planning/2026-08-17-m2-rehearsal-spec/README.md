...
| P1.b | A **psql session as the database-owner role** (`neondb_owner` — the role production's own services run as) against the **branch** endpoint | `04` L93: the bootstrap runs "by the database-owner actor — on Neon, the project's database owner." This is the leg where Neon's role layer gets its verdict, so a lesser substitute measures nothing |
...
| SD-M2-3 | `svc_migration` rehearsal password via `ALTER ROLE` from the owner session, post-bootstrap (P5) | Neon API-managed role credential — deliberately NOT used for `svc_migration`: routing the runner's login through the managed layer would blur exactly the layer §3 is measuring |
