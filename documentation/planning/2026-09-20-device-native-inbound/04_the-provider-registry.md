...
6. **Docs and comments.** Every `deps.drive` reference becomes `deps.adapters` — the code at `media_sync.py:546`, `work_loop.py:531`, `:667`, `:670`, and the prose at `media_sync.py:13` ("duck-typed on `deps.adapters[provider]`"), `work_loop.py:647` and `google_drive_adapter.py:13`; `.claude/rules/scheduler.md` (the worker page) names `deps.adapters`.
