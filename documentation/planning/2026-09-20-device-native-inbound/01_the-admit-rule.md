...
- `src/services/target/category_mix.py:41` `SUM_TOLERANCE = 0.001`; `:43` `MAX_SOURCES = 500`; `:47` `_LABEL` (`COALESCE(folder_name, folder_ref, 'folder')`); `:50-56` `MixInvalid` with the reason vocabulary `not_a_list · empty_source · duplicate_source · bad_ratio · sum_not_one · too_many_sources · unknown_source · all_off · ambiguous_name`; `:59-71` `_ratio` (NaN guard, four places, [0, 1]); `:74-78` `_sum_to_one`; `:81-103` `normalize`.
