...
- `src/api/routes/v1.py:627-660` — the routes (`:639` GET shape `mix`/`categories`; `:648-652` the `mix` body); `src/api/app.py` `MixInvalid` handler (400, `reason = invalid_mix_<why>`); `:232-234` the unmapped 500.
...
- `landing/src/lib/category-mix.ts:12,28-37,99` — the live card reads `data.mix` on GET and on the PUT response, and `categories` as `{category, media_count}` objects with a `null` root row (`category-weights-card.tsx:47-48`).
- `src/services/target/scheduler.py:448` — today's fallback query has no category predicate; `src/services/target/publish_pipeline.py:897` — the 7-day `recent` lock.

## Implementation Plan

### Dependencies
Phase 1b (registry rows and `category_id` columns exist; a walk has completed). F4 is locked (2026-09-07): proportional pool capped at the smallest explicit weight, plus a debut post.
...
1. **Planner** (`scheduler.execute_plan_slot`): read active registry rows joined to current mix rows by id — `SELECT c.id, c.name, c.debuted_at, x.ratio FROM media_categories c LEFT JOIN category_post_case_mix x ON x.workspace_id = c.workspace_id AND x.category_id = c.id AND x.effective_to IS NULL WHERE c.workspace_id = :ws AND c.state = 'active'`; count eligible media by `m.category_id` (the existing per-account eligibility). Rows: **explicit** (ratio > 0), **off** (ratio = 0: never drawn, excluded from automatic maths), **automatic** (no mix row). The F4 rule (locked), computed over the rows that have eligible media after Off, gone and id-less media are excluded and explicit rows with no eligible media are dropped: `share_auto` = eligible media in automatic rows ÷ eligible media in all remaining rows; `r_min` = the smallest ratio among the remaining explicit rows, renormalised among themselves; `A = min(share_auto, r_min / (1 + r_min))` (the cap is the smallest explicit weight on the final split: 70/30 → ≤ 23 %, a single row at 100 → ≤ 50 %); with no explicit rows `A = 1`; automatic rows split `A` by media share; explicit rows split `1 − A` by ratio. **Debut:** before the weighted draw, claim one automatic row with eligible media atomically — `UPDATE media_categories SET debuted_at = now() WHERE workspace_id = :ws AND id = (SELECT id … WHERE debuted_at IS NULL AND <eligible> ORDER BY first_seen_at, id LIMIT 1) AND debuted_at IS NULL RETURNING id` — and draw from it when the claim returns a row (two accounts in one slot cannot both debut the same folder). Draw over rows with eligible media; pick `WHERE m.category_id = :category_id`; keep the rotation order and the locks from #1251; `rng` stays injectable. **Fallback:** when the weighted set has no eligible media, the pool oldest-first as today (`:430`) **minus media whose `category_id` is a current Off row** (id-less and gone-row media stay in), so Off stays Off even when every weighted folder is locked out; all-Off is legal and posts nothing (`_notice_no_media` then says every folder is Off).
...
3. **Routes**: `GET /workspaces/{ws}/category-mix` → `{"rows": registry_view, "explicit_total": <sum of explicit ratios>, "mix": <current rows by name, the v1 shape>, "categories": <the v1 object shape `[{category, media_count}]` incl. the `null` root row, exactly as `v1.py:639` emits it today>}` — `test_v1_routes.py` pins the v1 keys until phase 3 removes them; `PUT` accepts `{"rows": [{"category_id", "ratio"}]}` and, until phase 3, the v1 `{"mix": [{"category", "ratio"}]}` body resolved by unique active name (`invalid_mix_ambiguous_name` when two active rows share a name); the response is the GET shape. Phase 3 removes `mix`/`categories` and the by-name body.
4. **Walk-completion notice** (in `media_sync.py` at the step-7 completion of phase 1b): once per completed walk, if any registry row of the source has `announced_at IS NULL` and media: the first completed walk says "Found N folders under <folder_name>: memes (3,474), merch (1,077), … — posting in proportion until you set weights in Settings › General › Category mix"; later walks say "New folder(s) under <folder_name>: events (100 files) — posts automatically (≈x %)"; 0-file rows are skipped and left unannounced; `announced_at` stamped on the rows named. One `fanout_notification` to the workspace's bindings per walk, never per row; rows are stamped `announced_at` only when at least one binding received the message, so a workspace that binds a group later still hears the summary on its next completed walk.
5. **Docs**: `06` §3 (the draw by id, the F4 rule, Off, the debut, the fallback), `03` ruling (F4 as re-locked, F8, F9), CHANGELOG.

## Test Plan

- Unit (`tests/src/services/target/test_work_loop.py`): the draw over ids; the F4 arithmetic (explicit 70/30 + a 10-file automatic folder; a 50,000-file automatic folder capped at 23 %; one explicit row at 100 + a 50,000-file archive capped at 50 %; an explicit row with no eligible media dropped before the cap; no explicit rows → proportional); Off rows never drawn and excluded from the maths; the debut draw and its stamp; a gone row never drawn; the fallback when the weighted set is empty excludes Off media and keeps id-less and gone-row media; all-Off posts nothing; two debut claims in one slot yield one debut; locks and rotation preserved.
- Unit (`test_category_mix.py`): validation by id (`unknown_category`, 0 allowed, sum-to-one over ratios > 0), gone rows accepted by `set_mix`, two sources' Unsorted rows weighted together (the cycle-1 B1 case: two current rows, one label), `registry_view`'s `effective` equals the planner's weights (one function, asserted by calling both), v1 `mix`/`categories` derived from the view.
