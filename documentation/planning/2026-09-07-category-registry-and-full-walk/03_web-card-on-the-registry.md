...
- `landing/src/components/dashboard/setup-wizard.tsx:28,439` — imports the legacy `category-mix-card.tsx` (`:10-19` its legacy shape).
...
4. API: remove `mix`/`categories` from the GET and the by-name `PUT` body (phase 2 step 3's transitional keys); `test_v1_routes.py` updated.
