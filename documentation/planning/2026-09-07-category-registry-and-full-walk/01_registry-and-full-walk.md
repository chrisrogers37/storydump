...
2. `media_sync.py`: the sync mints the token (uuid4) whenever the row holds no in-flight v2 cursor and passes the start cursor; a baseline or demand sync that finds a walk in flight joins it as a carrier. A **chunk carries one walk**: its payload names the walk, and a chunk that finds the row on another walk, or complete, or paused, returns without a provider call (`stale` / `paused`) — it never mints a walk. The cursor write is a **compare-and-swap** against the cursor this carrier read (`COALESCE(sync_checkpoint, '{}') = :old`), and a carrier whose CAS fails rolls back and writes nothing — no rows, no chunk. The summary line adds `walk`, `folders_seen` and `truncated`. The persistent-failure reset writes `{"v":2}`. Every `media_sources` statement in `_run_sync` carries `workspace_id` (the owner role bypasses RLS in production).
...
- 1a gate (`tests/scripts/test_w6_sync_gate.py::TestAChunkCarriesOneWalk`): the sync mints the token and hands the adapter a start cursor; a chunk for another walk, or after completion, or on a paused source, makes no provider call; a cursor moved under a carrier is not overwritten and the carrier writes nothing.
