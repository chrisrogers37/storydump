...
7. **Follow-up PR** (after the Vercel deploy is live): remove the v1 keys and the by-name refusal branch; `test_v1_routes.py` updated. Until then the OLD card cannot express Off: `v1_shape` drops Off rows, so a re-save from the old card turns an Off folder Automatic (review of the weights PR — accepted for the compat release).
