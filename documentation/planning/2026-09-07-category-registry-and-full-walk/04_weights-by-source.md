...
- Pins for the small migration: `tests/scripts/test_advertised_ddl.py:279` (27 → 28), `tests/scripts/test_lineage_lane.py` (+070), `src/models/target/intent_ledger.py:56,74-80` (the model and its by-name index); the tenancy lane handles `ADD COLUMN` and accepts `DROP INDEX`/`CREATE UNIQUE INDEX`.

## Implementation Plan

### Dependencies
`01_registry-and-full-walk.md` §1a merged (the walk), which also adds `media_items.folder_path TEXT NULL` (current path under the connected folder, refreshed each walk) — the only column 1a adds.
