...
RULES: tuple[Rule, ...] = (
    ...
    Rule(
        ...
        permits="TWO instagram tokens identical on the index key for one chat."
        " The obvious near-miss — one instagram token beside the google_drive"
        " one — differs in `service_name`, which is a KEY column, so that pair"
        " never collides whether the predicate is there or not: it discriminates"
        " on the key and says nothing about the WHERE clause. Measured — with"
        " the predicate stripped from migration 015 itself, that version passed"
        " and the whole file stayed green. The pair below is identical on the"
        " key and excluded by the predicate, so it survives only while the"
        " predicate is doing its job",
        ...
        near_miss=lambda c: [
            _token(c, "instagram", "oauth_refresh", chat=CHAT_A),
            _token(c, "instagram", "oauth_refresh", chat=CHAT_A),
        ],
    ),
    Rule(
...
class TestTheNearMissAssertionsAreSensitiveToWhatTheyClaim:
    """Controls on the controls — and their names say so now, because the
    earlier ones did not (#951 review, navi).

    **These do NOT test the migration files.** Each drops the shipped rule and
    installs a broken one *inside the test*, so it passes whatever
    `scripts/migrations/021_*.sql` actually contains. Named
    `test_undoing_021s_predicate_is_caught`, that was a promise the test could
    not keep: a test named for a regression is read as covering it.

    What they DO establish is worth keeping — that the corresponding
    `permits` assertion is sensitive to the property it is about, rather than
    passing for some unrelated reason.

    **The real-regression coverage lives in `TestEachRulePermitsItsNearMiss`,
    and that is measured rather than asserted.** Stripping the predicate from
    the real migration and replaying:

    ```
    021  WHERE locked_until IS NULL removed      -> near_miss[unique_permanent_lock_per_media]  FAILS
    040  auth_method dropped from the key        -> near_miss[unique_credential_per_account]    FAILS
    015  WHERE service_name='google_drive' gone  -> near_miss[unique_google_drive_token_per_chat] FAILS
    ```

    The 015 row only became true in this review cycle: its near-miss used to
    differ from the setup row in `service_name`, a KEY column, so the pair never
    collided predicate or no predicate and **a real 015 regression was caught by
    nothing at all** — the whole file stayed green. That is the defect these
    honest names exist to stop hiding.
    """

    def test_the_021_near_miss_is_sensitive_to_the_predicate(self, conn):
    ...
    def test_the_040_near_miss_is_sensitive_to_the_key_width(self, conn):
    ...
    def test_the_015_near_miss_is_sensitive_to_the_predicate(self, conn):
    ...
    def test_the_primary_key_exclusion_is_load_bearing(self, conn, monkeypatch):
        """`EXCLUDED_SUFFIX` silently shrinks the denominator, so it needs a
        control or it is an unexamined filter — the exact shape this file
        exists to refuse.

        **This test was CLAIMED before it existed** (#951 review, navi). I ran
        the mutation by hand during development, saw it redden coverage, and
        wrote it up in the CHANGELOG, the PR body and the commit message as
        though it had shipped. It had not. A claimed control is worse than an
        absent one: an absent one gets noticed.
        """
        baseline = enumerate_uniqueness_rules(conn)
        assert not any(n.endswith("_pkey") for n in baseline), (
            "the exclusion is not being applied at all"
        )

        monkeypatch.setattr(
            sys.modules[__name__], "EXCLUDED_SUFFIX", "__never_matches__"
        )
        widened = enumerate_uniqueness_rules(conn)

        pks = {n for n in widened if n.endswith("_pkey")}
        assert pks, (
            "no primary keys in this schema — the exclusion would be vacuous and"
            " this control would prove nothing"
        )
        assert widened - baseline == pks, (
            "neutralising the exclusion changed the denominator by something"
            " other than the primary keys"
        )
        assert not (pks & set(BY_INDEX)), (
            "a primary key has a behaviour case, so coverage would NOT fail on"
            " the widened denominator and the exclusion is doing nothing"
        )
