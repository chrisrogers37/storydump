...
def _credential(world, *, state="active", expires_at=None, ids=None):
    """Set the WORKSPACE's gdrive credential straight in — one row per
    workspace since 069 (`uq_credential_per_workspace`), so this upserts: each
    test below puts the workspace's grant in the state it needs."""
    ids = ids or world["a"]
    ...
    async def go(s):
        await s.execute(
            text(
                ...
                " (workspace_id, provider, encrypted_payload, state, expires_at)"
                " VALUES (:ws, 'gdrive', :payload, :state, :exp)"
...
def _drive(world, ids=None) -> dict:
    ids = ids or world["a"]
    return asyncio.run(
        in_tenant(
            world["ingress"],
            str(ids["ws"]),
            str(ids["user"]),
            lambda s: workspaces.drive_status(s, workspace_id=str(ids["ws"])),
        )
    )


class TestTheWorkspaceDriveStatus:
    """Since 069 (#1165) "is Google connected" is the WORKSPACE's question:
    one grant, every folder under it. `media_sources.state` still cannot
    answer it — a folder picked and then paused is `paused`, a healthy one
    `active` — and the source list no longer pretends to: it carries the
    folder, and `drive_status` carries the grant."""

    def test_a_workspace_with_no_grant_reads_none(self, world):
        b = world["b"]
        assert _drive(world, b) == {"status": "none", "connected_at": None}

    def test_a_grant_reads_active_and_carries_when_it_was_granted(self, world):
        _credential(world)
        status = _drive(world)
    ...
    def test_a_revoked_grant_reads_revoked_not_none(self, world):
        ...
        _credential(world, state="revoked")
        assert _drive(world)["status"] == "revoked"

    def test_the_source_list_carries_the_folder_and_no_credential(self, world):
        _credential(world, state="active")
        sid, _ = source(world, folder="stat-folder")
        row = next(r for r in _sources(world) if str(r["id"]) == str(sid))
        assert row["folder_ref"] == "stat-folder"
        assert "folder_name" in row
        assert "credential_status" not in row and "credential_connected_at" not in row

    def test_no_token_or_envelope_is_ever_returned(self, world):
        _credential(world, state="active")
        for row in [_drive(world)] + _sources(world):
