...
check "account answers for another workspace's handle" $OV '    " WHERE a.workspace_id = :ws"
    ...
    "   AND (a.id::text = :key"' "$GATE" "$BYPASS"
check "jobs counts the fleet, system rows included" $OV '    " FROM jobs j WHERE j.workspace_id = :ws"' '    " FROM jobs j WHERE true"' "$GATE" "$BYPASS"
check "outbox reads every binding" $OV '    " WHERE o.workspace_id = :ws"' '    " WHERE true"' "$GATE" "$BYPASS"
...
check "the handle must carry the @ to match" $OV '    "        OR lower(ltrim(a.handle, '"'"'@'"'"')) = lower(ltrim(:key, '"'"'@'"'"')))"' '    "        OR lower(a.handle) = lower(:key))"' "$GATE" "$GT -k returns_only_this_workspaces_rows"
check "the handle must match case" $OV '    "        OR lower(ltrim(a.handle, '"'"'@'"'"')) = lower(ltrim(:key, '"'"'@'"'"')))"' '    "        OR ltrim(a.handle, '"'"'@'"'"') = ltrim(:key, '"'"'@'"'"'))"' "$GATE" "$GT -k returns_only_this_workspaces_rows"
...
check "the window defaults to thirty hours" $VO 'DEFAULT_WINDOW = "3h"' 'DEFAULT_WINDOW = "30h"' "$UNIT" "$TU -k defaults_to_three_hours"
check "a weeks unit is accepted" $VO 'WINDOW_SPAN = re.compile(r"^(\d{1,6})([mhd])$")' 'WINDOW_SPAN = re.compile(r"^(\d{1,6})([mhdw])$")' "$UNIT" "tests/src/services/target/test_vocabulary.py -k everything_else_is_refused"
check "a naive timestamp is refused" $VO '            if parsed.tzinfo is None:
                parsed = parsed.replace(tzinfo=dt.timezone.utc)' '            if parsed.tzinfo is None:
                raise ValueError("naive")' "$UNIT" "$TU -k naive_as_utc"
...
# schema-privilege check is never evaluated (the first cut of this mutation survived that way)
check "the ledger's table is found by name" $OV '        "SELECT has_schema_privilege(current_user, n.oid, '"'"'USAGE'"'"')"' '        "SELECT has_table_privilege(current_user, '"'"'runner.schema_migrations'"'"', '"'"'SELECT'"'"')"' "$GATE" "$GT -k every_ledger_state"
...
check "only the first workspace of a name is read" $RD '    named = [str(ws["id"]) for ws in listed if ws.get("name") == workspace]' '    named = [str(ws["id"]) for ws in listed if ws.get("name") == workspace][:1]' "$UNIT" "tests/storydump_cli/test_reads.py -k shared_by_two_workspaces_reads_both"
