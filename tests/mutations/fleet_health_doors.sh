...
# The landing filter is one spelling in two places: the module's constant and the door's body.
check "the module's landing filter drifts from the door's" src/services/target/posting_health.py "_REAL_POST = \"state = 'posted' AND published_via NOT IN ('legacy_backfill', 'dry_run')\"" "_REAL_POST = \"state = 'posted' AND published_via <> 'legacy_backfill'\"" "$GATE -k door_filters_landings_the_way_the_module_says"
check "the door's landing filter drifts from the module's (the file)" scripts/migrations/081_fleet_health_doors.sql "  SELECT count(*) FILTER (WHERE state = 'posted' AND published_via NOT IN ('legacy_backfill', 'dry_run'))," "  SELECT count(*) FILTER (WHERE state = 'posted' AND published_via <> 'legacy_backfill')," "$GATE -k door_filters_landings_the_way_the_module_says"
