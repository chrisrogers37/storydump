...
- **Telegram button reactivity: media transfers no longer park the bot event loop, so callback acks fire immediately (#557)** — Clicking a post button (Auto Post / Posted / Skip / Reject) oftentimes hung — Telegram floated "Loading…", the button pulsed for ~10s, and sometimes never resolved (`Query is too old and response timeout expired`). Root cause: the worker runs a single asyncio event loop, and the synchronous Cloudinary/Drive media transfers ran **on** that loop. While any transfer was in flight the loop was parked, so every pending `answerCallbackQuery` — including acks for *other* users' clicks — was delayed past Telegram's callback-validity window and the spinner never cleared. Fix: offload the blocking media transfers to a worker thread (`asyncio.to_thread`) at all three on-loop sites — the **Auto Post** upload (`_upload_to_cloudinary`), the scheduled-notification photo download (`send_notification`), and the scheduler auto-approve download + upload (`_auto_approve_instagram`) — so the loop stays free to service acks while a transfer runs. Each offloaded call touches only its own task's cloud client / session, so the offload is safe with no change to the shared-session model. Also bounds the previously-unbounded Cloudinary upload with a finite network timeout (`CLOUD_UPLOAD_TIMEOUT_SECONDS`, default 120s) so a stalled upload fails fast instead of pinning a worker thread indefinitely. Behaviorally verified: with a slow upload in flight, five concurrent button-tap acks are serviced in ≤2 ms, where the loop previously parked ~0.9 s per transfer. This does **not** enable `concurrent_updates` or change the DB-session model — the callback handlers share singleton repositories (one SQLAlchemy `Session`) via the `atomic_session` primitive, which makes concurrent DB access unsafe until sessions are isolated per callback; that isolation plus `concurrent_updates` for true parallel throughput is tracked as a follow-up.
