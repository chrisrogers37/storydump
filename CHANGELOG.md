...
- **Media selection is tenant-scoped and fail-closed (#542), on a media-ownership enabler (#412)** — Closes the last cross-tenant hole in the JIT posting path, unblocking the #542 scoping that was deliberately sequenced behind media ownership.
...
- **Worker notification layer routes every tenant to its own chat (#541)** — Three cross-tenant holes in the worker/notification layer, closed by resolving the tenant from the data it owns instead of the deployment-wide env chat.
  ...
  - **Deliberately not addressed here (sequenced):** scheduler JIT selection / category-mix / queue-preview scoping (#542) stays blocked behind the media-ownership backfill (#412) — scoping selection while media rows are NULL-owned would empty every tenant's eligible pool and halt posting.
