...
- **F.4 (#751): the RLS runtime harness — the 58-policy schema is now EXERCISED as the declared runtime logins, not asserted present.** `tests/scripts/test_rls_runtime_harness.py` (43 tests) replays the full advertised stream under the declared actors and then acts as `svc_worker`/`svc_ingress` against it: a 17-table × 3-way tenant-isolation matrix (own-context positive control, foreign-context zero, absent-context zero — every zero paired with a positive so no assertion can pass vacuously), cross-tenant INSERT/UPDATE refusals, all nine SECURITY DEFINER doors exercised positive-as-permitted and denied-as-other (per real signatures — EXECUTE is per-signature), direct DELETE shut at the grant layer everywhere, auth-plane maintenance possible only through `fn_auth_plane_sweep` (with the deliberate `service_tokens` survivor proving the sweep does not overreach), `SET ROLE` refused from the runtime logins with the two bootstrap grants pinned to exactly their documented shape, and the zero-NULL tenant-key gates. Completeness is catalog-derived twice — the GUC-reading table set and the door/grant census are read from `pg_policies`/`pg_proc` and asserted equal to what the module exercised, so a new policy or door this file never touches is a red test, not a silent gap (mutation-proven: removing a table from the module registry reddens the census naming the drift). Also asserted: the real schema's absent-context failure shape is an EMPTY SET (NULLIF-wrapped `current_setting(..., true)`), deliberately different from the probe harness's 42704 — and the session-`SET` reuse leak reproduces on the production schema. The transaction-reuse leakage class itself (4 tests, measured SQLSTATEs, including the silent SET-LOCAL-reverts-to-previous-tenant case the spec missed) is recovered from the parked `feat/751-f4-transaction-reuse-leakage` branch; mason's #789 minors land with it (owner-side not-superuser/not-BYPASSRLS control; the FORCE mutant report). Out of CI reach and stated: the runtime-env credential-locality clause is a deployment property for the M-phase ops gate.
