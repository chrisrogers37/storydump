...
- **The lane tenancy check is prefix-aware, so F.2.2 lands green for the right reason (#806)** — `expected_tenancy()` in `scripts/tenancy_gate.py` derives the tenancy state a prefix of the advertised stream implies, and `test_lineage_lane.py` now asserts the replayed lineage **equals** it rather than asserting `tenant_keyed_tables(sig) == []`.
  ...
  - **Forward-only, and it refuses rather than guesses.** A `DROP POLICY` or `DISABLE ROW LEVEL SECURITY` in the stream would make the derivation overstate what is present — the quiet direction — so it raises instead. Mutation-proven: five mutants on `expected_tenancy` (tenant-key detection, the `workspaces` tenant-root case, policy counting, RLS recording, the teardown guard) each killed by the tests that should own them.
