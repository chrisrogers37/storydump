...
- **Instagram OAuth handles `/me/accounts` pagination and preserves the chat's active account on re-auth** — Followups on the multi-account ingest: (1) `/me/accounts` Graph endpoint paginates at ~25 Pages, so users with many FB Pages were silently losing accounts past the first response page. The Pages loop now walks `paging.next` until exhausted. (2) Previous behavior set `set_as_active=True` on the first detected IG every OAuth run, which could silently flip a user's chosen active away if Meta returned a different IG first. The active flag is now only set when the chat has no current active account (first connect, or after the previous active was removed). (3) Per-iteration `try/except` so one bad row doesn't drop the others; `account_count` in the result now reflects what actually persisted.
