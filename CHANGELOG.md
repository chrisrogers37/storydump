...
- **Instagram tokens expire silently because TokenRefreshService is never called** -- The `TokenRefreshService.refresh_all_instagram_tokens()` method existed but nothing in the scheduler loop invoked it. Tokens would expire after 60 days with no proactive refresh. Added a daily `_token_refresh_tick` to `scheduler_loop.py` that calls `refresh_all_instagram_tokens()` every 1440 ticks (24 hours), refreshing any token within the 7-day expiry buffer. Closes #397.
- **Concurrent token refresh race condition** -- When web and worker processes attempted to refresh the same token simultaneously, one could overwrite the other's freshly-stored token with a stale refresh result. Added `TokenRepository.get_token_for_update()` using `SELECT ... FOR UPDATE SKIP LOCKED` -- the refresh flow now acquires a row lock before calling Meta's API, and concurrent processes skip tokens already being refreshed rather than clobbering. Closes #375.
- **Refresh token revocation indistinguishable from access token expiry** -- When Instagram revokes the refresh token (user changes password, deauthorizes app), the error was caught as a generic `TokenExpiredError`, triggering silent retry loops instead of surfacing "please reconnect." Added `TokenRevokedError` exception class that detects Meta error subcodes 458 (app deauthorized), 460 (password changed), and 467 (token invalidated). `_check_response_errors` now raises `TokenRevokedError` for these subcodes, and the autopost error handler shows a distinct "account disconnected" message. Closes #377.
...
- **Media sync marks failed files as seen, preventing deactivation on retry** — `_process_provider_file()` added the file identifier to `seen_identifiers` before processing. If the file raised an exception, it was marked as "seen" but never actually processed — so `_deactivate_missing_items()` wouldn't flag it as missing on the next sync cycle. Now adds to `seen_identifiers` only after successful processing. Closes #365.
- **Stale processing queue items not recovered after crash** — `process_slot()` cleaned up stale pending items but not items stuck in "processing" without a Telegram message (from a crash mid-send). Added `requeue_stale_processing()` to reset these items back to "pending" after 10 minutes. Distinct from `discard_abandoned_processing()` which handles items that were sent to Telegram but never acted on. Closes #366.
- **SIGTERM during posting leaves queue items stuck in processing** — `_select_and_send()` could start a new Telegram send even after the shutdown handler set `shutdown_in_progress = True`. Now checks the flag before beginning work and returns immediately with `reason: shutdown_in_progress`. Closes #363.
- **Scheduler can post before first media sync completes** — On worker startup, the scheduler loop and media sync loop started independently. If the scheduler tick fired before any sync completed, it would either find no media or select from a stale set. Added a `session_state.initial_sync_complete` gate: the scheduler skips posting ticks until the media sync loop signals that at least one successful sync has finished. Closes #362.
